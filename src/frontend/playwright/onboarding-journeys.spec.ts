...
test('journey 1 runs simple small business onboarding flow', async ({ page }) => {
  ...
  await expect(page.locator('form').getByText('Small business details')).toBeVisible()

  await page.locator('#BusinessName').fill('Acme Local Ltd')
  await page.locator('#BusinessAddress').fill('1 High Street, London')
  await page.locator('#AnnualRevenue').fill('250000')
  await page.locator('#BusinessOwner').fill('Jamie Owner')
  await page.locator('#SanctionsDeclarationConfirmed').check()
  await page.getByRole('button', { name: 'Submit' }).click()

  await expect(page.locator('section.rounded-lg').getByText('Small business onboarding checks complete.')).toBeVisible()
})
...
test('journey 2 reaches medium business document verification step', async ({ page }) => {
  ...
  await page.locator('#PrimaryAddress').fill('200 King Road, Manchester')
  await page.locator('#AnnualRevenue').fill('3500000')
  await page.locator('#BusinessOwner').fill('Jordan Owner')
  await page.locator('#SecondaryBusinessOwner').fill('Taylor Coowner')
  await page.locator('#NumberOfOutlets').fill('18')
  await page.locator('#BusinessStaffCount').fill('120')
  await page.locator('#BeneficialOwnersConfirmed').check()
  await page.locator('#TaxComplianceConfirmed').check()
  await page.getByRole('button', { name: 'Submit' }).click()
...
test('journey 3 runs large nationwide onboarding and compliance questions', async ({ page }) => {
  ...
  await page.locator('#LegalStructure').selectOption('PublicLimitedCompany')
  await page.locator('#PrimaryAddress').fill('500 Enterprise Way, Birmingham')
  await page.locator('#AnnualRevenue').fill('25000000')
  await page.locator('#BusinessOwner').fill('Alex Executive')
  await page.locator('#SecondaryBusinessOwner').fill('Sam Director')
  await page.locator('#NumberOfOutlets').fill('220')
  await page.locator('#BusinessStaffCount').fill('2500')
  await page.getByRole('button', { name: 'Submit' }).click()

  await expect(page.locator('form').getByText('Compliance questionnaire')).toBeVisible()
  await page.locator('#RegulatoryLicensesConfirmed').check()
  await page.locator('#SanctionsScreeningCompleted').check()
  await page.locator('#BeneficialOwnershipReviewed').check()
  await page.getByRole('button', { name: 'Submit' }).click()

  await expect(page.locator('section.rounded-lg').getByText('Large nationwide business onboarding checks complete.')).toBeVisible()
})
