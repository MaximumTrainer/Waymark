...
internal sealed class SubmitStepCommandHandler(
    OnboardingDbContext dbContext,
    IValidator<SubmitStepRequest> validator,
    IComplianceRuleEvaluator complianceRuleEvaluator,
    ILogger<SubmitStepCommandHandler> logger,
    ...
    IPublisher publisher) : IRequestHandler<SubmitStepCommand, SessionStepResponse>
{
    private readonly JsonSerializerOptions _jsonOptions = new(JsonSerializerDefaults.Web);
    private readonly IReadOnlyList<ILogicNodeExecutor> _logicNodeExecutors = logicNodeExecutors.ToList();

    public async Task<SessionStepResponse> Handle(SubmitStepCommand command, CancellationToken cancellationToken)
    {
        var (sessionId, nodeId, request) = command;
        ...
        {
            if (nextNode is null)
                await publisher.Publish(new SessionCompletedEvent(session.Id, session.FlowId, session.UpdatedAt), cancellationToken);
            else
